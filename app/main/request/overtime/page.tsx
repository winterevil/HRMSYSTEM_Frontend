...
// Định nghĩa kiểu dữ liệu cho yêu cầu OT
...
export default function OvertimeRequest() {
    // Modal hiện đang ở chế độ "add" hay "edit"
    const [modalMode, setModalMode] = useState<"add" | "edit">("add");
    // Yêu cầu OT đang được xử lý trong modal process
    const [processRequest, setProcessRequest] = useState<OvertimeRequestDto | null>(null);

    // Danh sách yêu cầu OT
    ...
    // Tải dữ liệu ngày giờ theo định dạng
    ...
    function filterByMonth(requests: OvertimeRequestDto[], monthOffset = 0) {
    ...
    function calcStats(requests: OvertimeRequestDto[]) {
    ...
    const currentMonthRequests = filterByMonth(overtimeRequests, 0);
    const prevMonthRequests = filterByMonth(overtimeRequests, -1);
    ...
    const approvedHoursChange = comparePercent(currentStats.approvedHours, prevStats.approvedHours);
    ...
    function openEdit(request: OvertimeRequestDto) {
        setModalMode("edit");
        setCurrentRequest(request);
    }

    // Hàm mở modal xử lý
    function openProcess(request: OvertimeRequestDto) {
    ...
    useEffect(() => {
        ...
        async function loadData() {
            try {
                const reqRes = await fetch("https://localhost:7207/api/overtimerequest", { headers });
    ...
    async function handleSave() {
        ...
        try {
            ...
            if (modalMode === "add") {
                ...
                url = "https://localhost:7207/api/overtimerequest";
                method = "POST";
            } else {
                // Nếu là cập nhật
                body = {
            ...
            // reload danh sách
            const data = await fetch("https://localhost:7207/api/overtimerequest", { headers });
            ...
            setProcessRequest(overtimeRequests.find(r => r.id === processRequest?.id) || null);
        }
    }
    ...
    async function handleProcessSave() {
        ...
        try {
            // Gọi API
            const url = `https://localhost:7207/api/overtimerequest/approve/${processRequest.id}?status=${processRequest.status}`;
            const response = await fetch(url, {
                method: "POST",
                headers
            });
    ...
    return (
        <div className="section-body">
                            ...
                            <div className="row clearfix">
                                ...
                                <div className="col-lg-3 col-md-6">
                                    <div className="card">
                                        <div className="card-body">
                                            <h6>Total OT</h6>
                                            <h3 className="pt-3"><span className="counter">{currentStats.approvedHours.toFixed(1)}</span></h3>
                                            <span><span className={parseFloat(approvedHoursChange) >= 0 ? "text-success" : "text-danger"}>
                                                {approvedHoursChange}%
                                            </span> Since last month</span>
                                        </div>
                                    </div>
            ...
            {/* Modal thêm mới / chỉnh sửa yêu cầu OT */}
            <div className="modal fade" id="exampleModal" tabIndex={-1} role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div className="modal-dialog" role="document">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title" id="exampleModalLabel">{modalMode === "add" ? "Request Overtime" : "Process Overtime"}</h5>
                        ...
                        <div className="modal-body">
                            <div className="row clearfix">
                                ...
                                <div className="col-md-12">
                                    <div className="form-group">
                                        <input type="datetime-local" className="form-control" placeholder="Start Time"
                                ...
                                <div className="col-md-12">
                                    <div className="form-group">
                                        <input type="datetime-local" className="form-control" placeholder="End Time"
